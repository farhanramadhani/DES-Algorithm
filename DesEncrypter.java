...
public class DesEncrypter {
    ...
    private int[] listShift = new int[] {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};
    ...
    private int[] matrixPcDua = new int[] {14,17,11,24,1,53,28,15,6,21,10,23,19,12,4,26,8,16,7,27,
    ...
    public List<String> makeBlock(String plaintextBiner, int length) throws UnsupportedEncodingException {
    ...
    public String sBox(String keyBiner, List<int[][]> matrix) throws UnsupportedEncodingException {
        List<String> blocks = makeBlock(keyBiner,6);
        String block = "";
        int row;
        int col;
        String temp;
    ...
    public String encrypt(String plaintext, String key) throws UnsupportedEncodingException {
        List<String> plaintextBlocks = makeBlock(plaintext,64); //mengecek tiap blok harus 64bit
        ...
        for (String plaintextBlock : plaintextBlocks){ //tiap 64bit
            leftPlaintext = getLeftPlainBiner(plaintextBlock);
            rightPlaintext = getRighPlainBiner(plaintextBlock);

            key = permutation(keyToBiner(key), matrixPcSatu); //ubah ke biner dan di kompress ke 56bit
            for (int n=0; n<16; n++){
                internalKey = leftShift(getLeftKeyBiner(key), listShift[n]) + leftShift(getRightKeyBiner(key), n);//geser bit sebanyak n kali
                internalKey = permutation(internalKey, matrixPcDua); //permutasi dengan matriks PCdua atau K
                //internalKeys.add(internalKey);
                a = xor(internalKey,expand(rightPlaintext,matrixExpand));
                b = sBox(internalKey,this.sBoxes);
                b = permutation(b,pBox);
                leftPlaintext = rightPlaintext;
                rightPlaintext = xor(leftPlaintext,b);
            }
        }
